...
from pccai.utils.convert_octree import OctreeOrganizer, BlkOctArrayOrganizer, OctArrayOrganizer, OctVoxelContextOrganizer
from pccai.dataloaders.lidar_base_loader import VFBBase, FordBase, KITTIBase, QnxadasBase


def get_base_lidar_dataset(data_config, sele_config):
    if data_config['dataset'].lower().find('vfb') >= 0:
        loader_class = VFBBase
    elif data_config['dataset'].lower().find('ford') >= 0:
...
class LidarBlkOctArray(data.Dataset):
    '''
    A Ford dataset which partitions an original point cloud into an octree.
    '''
    ...
    def __init__(self, data_config, sele_config, **kwargs):
        ...
        self.point_cloud_dataset = get_base_lidar_dataset(data_config, sele_config)
        self.rw_octree = data_config.get('rw_octree', False)
        self.quant_scale = data_config.get('scale', 1)
        if self.rw_octree:
            self.rw_partition_scheme = data_config.get('rw_partition_scheme', 'default')
        self.octree_cache_folder = 'octree_cache'
        ...
        self.octree_organizer = BlkOctArrayOrganizer(
            data_config['octree_cfg'],
            data_config['max_num_points'],
            kwargs['syntax'].syntax_gt,
            kwargs['syntax'].syntax_gt_oct_array,
            self.quant_scale,
            self.rw_octree
        )
    ...
    def __getitem__(self, index):
        ...
        if self.rw_octree:
            file_name = os.path.relpath(self.point_cloud_dataset.get_pc_idx(index), self.point_cloud_dataset.dataset_path)
            file_name = os.path.join(self.point_cloud_dataset.dataset_path, self.octree_cache_folder, self.rw_partition_scheme, file_name)
            file_name = os.path.splitext(file_name)[0] + '_qs' + str(self.quant_scale) + '.pkl'
        else: file_name = None

        # perform octree partitioning and organize the data
        pc_formatted, _, _ = self.octree_organizer.organize_data(
            self.point_cloud_dataset[index]['pc'],
            file_name
        )

        return pc_formatted

class LidarOctArray(data.Dataset):
    '''
    A Ford dataset which partitions an original point cloud into an octree.
    '''
    ...
    def __init__(self, data_config, sele_config, **kwargs):
        ...
        # Create an octree formatter to organize octrees into arrays
        self.octree_organizer = OctArrayOrganizer(
            data_config['octree_cfg'],
            data_config['max_num_points'],
            kwargs['syntax'].syntax_gt,
            kwargs['syntax'].syntax_gt_oct_array,
...
class LidarOctVoxelContext(data.Dataset):
    '''
    A Ford dataset which partitions an original point cloud into an octree.
    '''
    ...
    def __init__(self, data_config, sele_config, **kwargs):
        ...
        self.point_cloud_dataset = get_base_lidar_dataset(data_config, sele_config)
        self.crm = data_config.get('coord_refine', False)
        self.upsample = data_config.get('up_sample', False)
        self.lossless = data_config.get('lossless', False)
        self.rw_octree = data_config.get('rw_octree', False)
        self.quant_scale = data_config.get('scale', 1)
        if self.rw_octree:
            self.rw_partition_scheme = data_config.get('rw_partition_scheme', 'default')
        ...
        if not self.crm:
            self.octree_organizer = OctVoxelContextOrganizer(
                ...
                self.quant_scale,
                self.rw_octree,
                data_config['normalized_vox_dist']
            )
        else:
            self.octree_organizer = OctCRMVoxelContextOrganizer(
                data_config['octree_cfg'],
                data_config['max_num_points'],
                kwargs['syntax'].syntax_gt,
                kwargs['syntax'].syntax_gt_oct_array,
    ...
    def __getitem__(self, index):
        ...
        # perform octree partitioning and organize the data
        pc_formatted, points_raw, points_gt = self.octree_organizer.organize_data(
            self.point_cloud_dataset[index]['pc'],
            file_name
        )

        if self.upsample:
            return pc_formatted, points_raw, points_gt
        else:
            return pc_formatted
