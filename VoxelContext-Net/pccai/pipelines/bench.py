...
import sys
sys.path.append(os.path.dirname(os.path.dirname(os.path.dirname(os.path.realpath(__file__))))+'/utils/')
from visualize import read_point_cloud
...
def benchmark_checkpoints(opt):
    ...
    for filename_ckpt in opt.checkpoints:
        ...
        for idx, pc_file in enumerate(pc_file_list):
            ...
            pc_raw = np.asarray(read_point_cloud(pc_file).points) #support both bin and ply, old: np.asarray(o3d.io.read_point_cloud(pc_file).points)
            log_dict = {
            ...
            if opt.remove_compressed_files:
                for f in compressed_files: os.remove(f) if 'CRM' not in str(pccnet.encoder.__class__) else None
            if '_gt.ply' in pc_file and '/tmp/' in pc_file: os.remove(pc_file)
...
def load_benchmark_config(opt):
    ...
    if opt.level_max is not None:
        codec_config['octree_cfg']['level_max'] = int(opt.level_max)
        codec_config['octree_cfg']['level_min'] = int(opt.level_max)
    if opt.scale is not None:
        codec_config['scale'] = opt.scale
